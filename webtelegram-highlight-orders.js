// web telegram
// highlight orders that are not in appsheet
...
function highlight_orders(ordersID_list) {
	const orders_list = document.querySelectorAll('.message.spoilers-container .anchor-hashtag')
	for (let i=0; i < orders_list.length; i++) {
		if ( !ordersID_list.includes(orders_list[i].textContent.slice(1)) ) orders_list[i].setAttribute('style', 'background-color: #fff200; padding: 10px;')
	}
}